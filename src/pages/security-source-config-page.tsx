...
import {
  ...
  updateSecurityDataSourceStatus,
...
const accessSecondaryNavItems: Array<{
  id: string
  label: string
  path: string
  icon: LucideIcon
...
function SourceSecondaryTabs({ withEmbed, actions }: { withEmbed: (path: string) => string; actions?: ReactNode }) {
  ...
  const activeView = location.pathname.endsWith('/rule-config')
    ? 'access-rules'
...
function SourceDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <aside className="absolute right-0 top-0 flex h-dvh max-h-dvh w-full max-w-[780px] flex-col overflow-hidden border-l border-[var(--line)] bg-[var(--surface)] shadow-[var(--shadow-strong)]">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 border-t border-[var(--line)] px-6 py-4">
          ...
          <Button variant="secondary" disabled={isSaving} onClick={() => void submit(true)}>保存为草稿</Button>
          <Button disabled={isSaving} onClick={() => void submit(false)}>{isSaving ? '正在保存...' : '保存并启用'}</Button>
        </div>
      </aside>
    </div>
  )
...
export function SecuritySourceConfigPage() {
  ...
  const testConnection = async (source: SecurityDataSourceRecord) => {
    setConnectionNotice('')
    if (source.sourceType !== 'third_party_api') {
      setConnectionNotice(`${source.name}：数据库连接检查需由后端 Workflow 执行，当前不会伪造成功结果。`)
      return
    }
    if (!/^https?:\/\//i.test(source.host)) {
      setConnectionNotice(`${source.name}：请先配置有效的 HTTP/HTTPS 接口地址。`)
      return
    }
    await updateSecurityDataSourceStatus(source.id, 'testing')
    const startedAt = performance.now()
    try {
      const response = await fetch(source.host, { method: 'HEAD' })
      const latencyMs = Math.round(performance.now() - startedAt)
      if (!response.ok) throw new Error(`HTTP ${response.status}`)
      await updateSecurityDataSourceStatus(source.id, 'connected', {
        ...source.monitor,
        latencyMs,
        lastHeartbeat: new Date().toISOString(),
        issue: '接口探测通过',
      })
      setConnectionNotice(`${source.name} 连接检查通过，延迟 ${latencyMs}ms。`)
    } catch (caught) {
      await updateSecurityDataSourceStatus(source.id, 'exception', {
        ...source.monitor,
        latencyMs: null,
        lastHeartbeat: new Date().toISOString(),
        issue: toErrorMessage(caught, '接口探测失败'),
      })
      setConnectionNotice(`${source.name} 连接检查失败：${toErrorMessage(caught, '网络或认证错误')}`)
    } finally {
      await refresh()
    }
  }
    ...
    <>
      <div className="space-y-5">
        ...
        <section className="rounded-[8px] border border-[var(--line)] bg-[var(--surface-raised)] p-4 shadow-[var(--shadow-soft)]">
          <div className="grid gap-3 xl:grid-cols-[minmax(260px,1.3fr)_repeat(2,minmax(150px,0.6fr))_auto]">
            <label className="flex h-10 min-w-0 items-center gap-2 rounded-[8px] border border-[var(--line)] bg-[var(--surface-muted)] px-3">
              ...
              <input value={keyword} onChange={(event) => { setKeyword(event.target.value); setPage(1) }} className="min-w-0 flex-1 bg-transparent text-[0.875rem] outline-none" placeholder="搜索数据源名称、类型、标签或接入规则" />
            </label>
            ...
            <div className="min-w-[1200px]">
              ...
              {visibleRows.map((row) => (
                <div key={row.id} className="grid grid-cols-[1.35fr_150px_140px_1.2fr_1.1fr_130px_150px] gap-3 border-b border-[var(--line)] px-4 py-4 text-[0.8125rem] last:border-b-0 hover:bg-[var(--surface-muted)]">
                  ...
                  <Link to={withEmbed('/security-governance/access-control/policy-engine')} className="line-clamp-2 text-[var(--primary)] hover:underline">{row.policyName || '暂未关联'}</Link>
