...
function createEmptySource(): EditableSecurityDataSource {
  return {
    ...
    <div className="fixed inset-0 z-50">
      ...
      <aside className="absolute inset-y-0 right-0 flex h-full max-h-[100dvh] w-full max-w-[780px] flex-col overflow-hidden border-l border-[var(--line)] bg-[var(--surface)] shadow-[var(--shadow-strong)]">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-5">
          ...
          <section className="space-y-3">
            <h3 className="text-[0.95rem] font-semibold text-[var(--text-main)]">接入安全</h3>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="flex items-center justify-between rounded-[8px] border border-[var(--line)] bg-[var(--surface-raised)] px-3 py-3 text-[0.8125rem] text-[var(--text-secondary)]"><span>启用数据抽样</span><input type="checkbox" checked={form.securityConfig.samplingEnabled} onChange={(event) => updateSecurity('samplingEnabled', event.target.checked)} /></label>
              <label className="space-y-1 text-[0.8125rem] text-[var(--text-secondary)]"><span>抽样率（%）</span><input type="number" min="1" max="100" className={inputClassName} value={form.securityConfig.samplingRate} onChange={(event) => updateSecurity('samplingRate', Number(event.target.value))} /></label>
            </div>
          </section>
...
export function SecuritySourceConfigPage() {
  ...
  const allTags = useMemo(() => Array.from(new Set(sourceRows.flatMap((row) => row.tags))).slice(0, 12), [sourceRows])
  const filteredRows = useMemo(() => {
    ...
    return sourceRows
      ...
      .filter((row) => {
        ...
        return [row.name, row.code, row.sourceTypeLabel, row.statusLabel, row.ownerDept, row.description, row.tags.join(' ')]
    ...
    <>
      <div className="space-y-5">
        <SourceSecondaryTabs
          actions={
            <>
              ...
              <Button className="gap-2" onClick={() => { setDrawerMode('create'); setDrawerSource(createEmptySource()) }}>
        ...
        <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-4">
          ...
          <MetricCard title="加密接入" value={encryptedCount.toLocaleString()} detail="统计已启用传输加密的数据源。" icon={<ShieldCheck className="h-5 w-5" />} tone="warning" />
        </div>
        ...
        <section className="overflow-hidden rounded-[8px] border border-[var(--line)] bg-[var(--surface-raised)] shadow-[var(--shadow-soft)]">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[1200px]">
              <div className="grid grid-cols-[1.35fr_150px_140px_1.2fr_130px_150px] gap-3 border-b border-[var(--line)] bg-[var(--surface-muted)] px-4 py-3 text-[0.75rem] font-medium text-[var(--text-muted)]">
                <span>数据源名称</span><span>数据源类型</span><span>连接状态</span><span>数据标签</span><span>最后检查</span><span>操作</span>
              </div>
              {visibleRows.map((row) => (
                <div key={row.id} className="grid grid-cols-[1.35fr_150px_140px_1.2fr_130px_150px] gap-3 border-b border-[var(--line)] px-4 py-4 text-[0.8125rem] last:border-b-0 hover:bg-[var(--surface-muted)]">
