...
轻量运行服务统一承担数据源连接检查、数据 API 路由、签名认证、在线策略、风险判断、量测查询编排和审计回写。
...
数据 API 默认使用 `X-API-Key` 和 `X-Scenario`。API Key 只通过 `SUBJECT_SECRETS_JSON` 注入，数据库只保存安全引用。运行顺序固定为：API Key 鉴权 → 主体 API 授权清单 → 资源访问策略 → 风险判断 → 数据访问。
